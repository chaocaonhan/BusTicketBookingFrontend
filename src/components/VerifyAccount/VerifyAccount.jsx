...
const VerifyAccount = () => {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      setMessage("Mã xác nhận không đúng hoặc đã hết hạn.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-900">
      <div className="w-full max-w-md p-6 bg-gray-800 rounded-lg shadow-md text-white">
        ...
        <p className="text-center text-gray-400">Please, enter it below to verify your email</p>
        {message && <div className="p-3 mt-3 text-sm text-blue-400 bg-gray-700 rounded-md">{message}</div>}
        ...
        <p className="mt-3 text-center text-gray-400">
          Didn't receive the code? <span className="text-blue-400 cursor-pointer">Send Again</span>
        </p>
      </div>
    </div>
  );
